    ...
    try:
        ...
        if code != 0:
            ...
            if result == '。' or result=='.。' or result==' .。' or result==' 。':
                ...
                t.insert(END, result)
                #jihe=[]
                if result not in jihe:
                    jihe.append(result)
                    #print(jihe)    #输出结果


                # print("翻译结果: %s。" % (result))   #输出结果(jihe变量是列表形式)
...
def run():
    global wsParam
    wsParam = Ws_Param(APPID='5fe88d4a',APISecret='5857f187d1b13bdb314d096ca25d846a', APIKey='893a2a06dfecca9127149e0c353cf8ab')
